...
const largestRoute = javascript.filter((file) => file.path !== entry.path && !file.path.includes('exercises-')).sort((a, b) => b.bytes - a.bytes)[0];
...
const checks = [
    ...
    {check: 'reviewed exercise data chunk', status: javascript.filter((file) => file.path.includes('exercises-')).every((file) => file.bytes <= 900_000) ? 'pass' : 'fail', detail: 'lazy and below 900000 bytes'},
];
