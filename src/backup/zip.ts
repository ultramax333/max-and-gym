...
function safePath(path: string): void {
    if (!path || path.length > 240 || path.startsWith('/') || path.startsWith('\\') || /^[a-z]:/i.test(path) || path.split(/[\\/]/).includes('..') || path.includes('\\') || /\0/.test(path)) throw new ArchiveError('ARCHIVE_PATH_INVALID', 'Chemin d’archive non sûr.');
}
...
export function encodeZip(entries: ZipEntry[]): Uint8Array {
    if (entries.length > 2000) throw new ArchiveError('ARCHIVE_LIMIT_EXCEEDED', 'Trop de fichiers.');
    ...
    for (const entry of entries) {
        safePath(entry.path);
        if (seen.has(entry.path)) throw new ArchiveError('ARCHIVE_FORMAT_INVALID', 'Chemin dupliqué.');
        seen.add(entry.path);
...
export function decodeZip(input: ArrayBuffer | Uint8Array, limits = {maxFiles: 2000, maxBytes: 256 * 1024 * 1024}): ZipEntry[] {
    ...
    if (bytes.length < 22 || bytes.length > limits.maxBytes) throw new ArchiveError('ARCHIVE_LIMIT_EXCEEDED', 'Archive vide ou trop grande.');
    ...
    if (endOffset < 0) throw new ArchiveError('ARCHIVE_FORMAT_INVALID', 'Fin ZIP absente.');
    const count = view.getUint16(endOffset + 10, true);
    const centralOffset = view.getUint32(endOffset + 16, true);
    if (count > limits.maxFiles) throw new ArchiveError('ARCHIVE_LIMIT_EXCEEDED', 'Trop de fichiers.');
    const entries: ZipEntry[] = [];
    const seen = new Set<string>();
    let cursor = centralOffset;
    let expanded = 0;
    for (let index = 0; index < count; index++) {
        if (cursor + 46 > endOffset || view.getUint32(cursor, true) !== 0x02014b50) throw new ArchiveError('ARCHIVE_FORMAT_INVALID', 'Répertoire ZIP invalide.');
        ...
        if (method !== 0 || cursor + 46 + nameLength + extraLength + commentLength > endOffset) throw new ArchiveError('ARCHIVE_FORMAT_INVALID', 'Compression ZIP non prise en charge.');
        const path = decoder.decode(bytes.slice(cursor + 46, cursor + 46 + nameLength));
        safePath(path);
        if (seen.has(path)) throw new ArchiveError('ARCHIVE_FORMAT_INVALID', 'Chemin dupliqué.');
        seen.add(path);
        if (localOffset + 30 > bytes.length || view.getUint32(localOffset, true) !== 0x04034b50) throw new ArchiveError('ARCHIVE_FORMAT_INVALID', 'Entrée ZIP locale invalide.');
        const localNameLength = view.getUint16(localOffset + 26, true);
        const localExtraLength = view.getUint16(localOffset + 28, true);
        const dataOffset = localOffset + 30 + localNameLength + localExtraLength;
        if (dataOffset + size > bytes.length) throw new ArchiveError('ARCHIVE_FORMAT_INVALID', 'Entrée ZIP tronquée.');
        const data = bytes.slice(dataOffset, dataOffset + size);
        if (crc32(data) !== expectedCrc) throw new ArchiveError('ARCHIVE_CHECKSUM_INVALID', 'CRC ZIP invalide.');
        expanded += data.length;
        if (expanded > limits.maxBytes) throw new ArchiveError('ARCHIVE_LIMIT_EXCEEDED', 'Archive décompressée trop grande.');
