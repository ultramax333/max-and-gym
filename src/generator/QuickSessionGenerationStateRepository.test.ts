...
describe('quick-session generation state', () => {
    ...
    it('persists a monotone variation and the two latest generations per zone', async () => {
        await repository.record('arms', 1, ['curl-a', 'extension-a']);
        await repository.record('arms', 2, ['curl-b']);
        const state = await repository.record('arms', 3, ['curl-c']);
        expect(state).toEqual({nextVariation: 4, recentGenerations: [['curl-c'], ['curl-b']]});
