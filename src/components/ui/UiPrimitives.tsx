...
export function ScreenContainer({children}: {children: ReactNode}) {
    return <Box sx={{width: '100%', maxWidth: 1160, mx: 'auto', px: {xs: 2, sm: 3, lg: 4}, py: {xs: 2.5, sm: 3.5}, pb: {xs: 'calc(104px + env(safe-area-inset-bottom))', md: 5}}}>{children}</Box>;
}

export function SectionHeader({eyebrow, title, action}: {eyebrow?: string; title: string; action?: ReactNode}) {
    return <Stack direction={{xs: 'column', sm: 'row'}} justifyContent="space-between" alignItems={{xs: 'stretch', sm: 'flex-end'}} gap={1.5} sx={{mb: 2.5}}><Box sx={{minWidth: 0}}>{eyebrow && <Typography variant="overline" color="primary.main" sx={{display: 'block', mb: 0.25}}>{eyebrow}</Typography>}<Typography component="h1" variant="h4">{title}</Typography></Box>{action && <Box sx={{flexShrink: 0}}>{action}</Box>}</Stack>;
}
...
export function StatePanel({title, description, action, icon = <Inbox/>}: {title: string; description: string; action?: ReactNode; icon?: ReactNode}) {
    return <Paper sx={{p: {xs: 3, sm: 4}, textAlign: 'center', borderStyle: 'dashed', borderRadius: '24px', background: 'radial-gradient(circle at 50% 0%, rgba(83,199,183,.1), transparent 45%), #101720'}}><Stack spacing={1.5} alignItems="center"><Box sx={{width: 56, height: 56, display: 'grid', placeItems: 'center', borderRadius: '18px', color: 'primary.main', bgcolor: 'rgba(83,199,183,.12)', '& svg': {fontSize: 28}}}>{icon}</Box><Typography component="h2" variant="h6">{title}</Typography><Typography color="text.secondary" sx={{maxWidth: 440}}>{description}</Typography>{action && <Box sx={{pt: 0.5}}>{action}</Box>}</Stack></Paper>;
}
