...
import {buildPersonalBackup, BackupPreview, importPersonalBackup, previewPersonalBackup, recordPersonalBackupSuccess} from '../../backup/PersonalBackupService';
...
import {backupDocumentGateway} from '../../native/backupDocumentGateway';
...
export default function BackupPage() {
    ...
    const exportBackup = async () => { try { setBusy(true); setError(''); setMessage(''); const fileName = `max-and-gym-${new Date().toISOString().replace(/[:.]/g, '-')}.maxgym`; if (backupDocumentGateway.isNativeAndroid()) { const blob = await buildPersonalBackup(db, {recordSuccess: false}); const saved = await backupDocumentGateway.save(blob, fileName); if (!saved) { setMessage('Backup cancelled. No file was written.'); return; } await recordPersonalBackupSuccess(db); setMessage('Backup saved to the selected location.'); } else { const blob = await buildPersonalBackup(db); downloadBlob(blob, fileName); setMessage('Backup validated and ready to download.'); } } catch (reason) { setError(reason instanceof Error ? reason.message : 'Could not create backup.'); } finally { setBusy(false); } };
