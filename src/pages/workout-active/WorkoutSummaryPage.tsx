...
export function WorkoutSummaryPage() {
    ...
    return <Layout title="Résumé de séance" hideNav hideBack><ScreenContainer><Stack spacing={2}><StatePanel title="Séance enregistrée" description="La session est terminée et conservée localement sur cet appareil." icon={<CheckCircle/>}/><Stack direction={{xs: 'column', sm: 'row'}} gap={2}><Card sx={{flex: 1}}><CardContent><Typography color="text.secondary">Séries</Typography><Typography variant="h4">{completed.length}/{snapshot.sets.length}</Typography></CardContent></Card><Card sx={{flex: 1}}><CardContent><Typography color="text.secondary">Durée active</Typography><Typography variant="h4">{Math.floor(duration / 60)} min</Typography></CardContent></Card><Card sx={{flex: 1}}><CardContent><Typography color="text.secondary">Volume</Typography><Typography variant="h4">{Math.round(volume)} kg</Typography></CardContent></Card></Stack><Stack direction={{xs: 'column', sm: 'row'}} gap={1}><SecondaryButton onClick={() => navigate('/progress/proposals')}>Voir les propositions</SecondaryButton><PrimaryButton onClick={() => navigate('/')}>Retour à l’accueil</PrimaryButton></Stack></Stack></ScreenContainer></Layout>;
}
