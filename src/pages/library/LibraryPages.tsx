...
function ExerciseCard({exercise, onOpen, onToggleFavourite, onToggleNeverSuggest}: {exercise: LibraryExercise; onOpen: () => void; onToggleFavourite: () => void; onToggleNeverSuggest: () => void}) {
    ...
    return <Card sx={{overflow: 'hidden'}}><CardActionArea onClick={onOpen}><Stack direction="row">{thumbnail ? <CardMedia component="img" image={mediaUrl(thumbnail.path)} alt={thumbnail.altText} loading="lazy" sx={{width: {xs: 112, sm: 128}, minHeight: 128, bgcolor: 'background.default', objectFit: 'contain', borderRight: '1px solid', borderColor: 'divider'}}/> : <Box sx={{width: {xs: 112, sm: 128}, minHeight: 128, flexShrink: 0, display: 'grid', placeItems: 'center', bgcolor: 'rgba(9,13,18,.68)', color: 'text.disabled', borderRight: '1px solid', borderColor: 'divider'}}><Stack alignItems="center" spacing={0.5}><FitnessCenter/><Typography variant="caption">No photo</Typography></Stack></Box>}<CardContent sx={{minWidth: 0, flex: 1}}><Stack direction="row" justifyContent="space-between" gap={1}><Box sx={{minWidth: 0}}><Typography variant="overline" color="primary.main">{exercise.primaryMuscles[0] ?? 'EXERCISE'}</Typography><Typography component="h2" variant="h6" sx={{overflow: 'hidden', textOverflow: 'ellipsis'}}>{exercise.name}</Typography></Box><Stack direction="row"><IconButton aria-label={exercise.effectiveNeverSuggest ? 'Excluded exercises cannot be favourites' : exercise.favourite ? 'Remove from favourites' : 'Add to favourites'} disabled={exercise.effectiveNeverSuggest} onClick={(event) => { event.stopPropagation(); onToggleFavourite(); }}>{exercise.favourite ? <Favorite color="error"/> : <FavoriteBorder/>}</IconButton><IconButton aria-label={exercise.neverSuggest ? `${exercise.name} is excluded by the catalogue` : exercise.effectiveNeverSuggest ? `Allow ${exercise.name} in suggestions` : `Never suggest ${exercise.name}`} color={exercise.effectiveNeverSuggest ? 'warning' : 'default'} disabled={exercise.neverSuggest} onClick={(event) => { event.stopPropagation(); onToggleNeverSuggest(); }}><Block/></IconButton></Stack></Stack><Stack direction="row" flexWrap="wrap" gap={0.5} sx={{mt: 1}}><EquipmentBadges exercise={{exerciseId: exercise.id, equipmentTags: exercise.equipmentTags}}/><Chip size="small" variant="outlined" label={exercise.movementPattern}/>{exercise.effectiveNeverSuggest && <Chip size="small" color="warning" icon={<Block/>} label={exercise.neverSuggest ? 'Catalogue exclusion' : 'Never Suggest'}/>}</Stack></CardContent></Stack></CardActionArea></Card>;
}
...
export function LibraryPage() {
    ...
    return <Layout title="Library" hideBack><ScreenContainer><Stack spacing={2}><SectionHeader eyebrow="LOCAL EXERCISE CATALOGUE" title="Library" action={!loading && <Chip color="primary" label={`${exercises.length} exercises`}/>}/><Stack direction={{xs: 'column', sm: 'row'}} gap={1}><TextField fullWidth label="Search exercises" value={filters.search ?? ''} onChange={(event) => setFilters({...filters, search: event.target.value || undefined})} InputProps={{startAdornment: <Search sx={{mr: 1, color: 'text.secondary'}}/>}}/><Button variant="outlined" startIcon={<FilterAlt/>} onClick={() => setFiltersOpen(true)}>Filters</Button><Button variant="outlined" startIcon={<Add/>} onClick={() => setCustomOpen(true)}>Create</Button></Stack>
        <Typography color="text.secondary">{loading ? 'Loading…' : 'Reviewed movements, local photos and your own preferences.'}</Typography>
