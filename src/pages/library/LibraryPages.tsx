...
export function LibraryPage() {
    ...
    return <Layout title="Bibliothèque" hideBack><ScreenContainer><Stack spacing={2}><Stack direction={{xs: 'column', sm: 'row'}} gap={1}><TextField fullWidth label="Rechercher un exercice" value={filters.search ?? ''} onChange={(event) => setFilters({...filters, search: event.target.value || undefined})} InputProps={{startAdornment: <Search sx={{mr: 1, color: 'text.secondary'}}/>}}/><Button variant="outlined" startIcon={<FilterAlt/>} onClick={() => setFiltersOpen(true)}>Filtres</Button><Button variant="outlined" startIcon={<Add/>} onClick={() => setCustomOpen(true)}>Créer</Button></Stack>
