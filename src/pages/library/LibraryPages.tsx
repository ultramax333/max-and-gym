...
function ExerciseCard({exercise, onOpen, onToggleFavourite}: {exercise: LibraryExercise; onOpen: () => void; onToggleFavourite: () => void}) {
    ...
    return <Card><CardActionArea onClick={onOpen}><Stack direction="row"><CardMedia component="img" image={thumbnail ? mediaUrl(thumbnail.path) : undefined} alt={thumbnail?.altText ?? ''} loading="lazy" sx={{width: 104, height: 104, bgcolor: 'background.default', objectFit: 'cover'}}/><CardContent sx={{minWidth: 0, flex: 1, py: 1.5}}><Stack direction="row" justifyContent="space-between" gap={1}><Typography component="h2" variant="h6" sx={{overflow: 'hidden', textOverflow: 'ellipsis'}}>{exercise.name}</Typography><Button aria-label={exercise.favourite ? 'Remove from favourites' : 'Add to favourites'} size="small" onClick={(event) => { event.stopPropagation(); onToggleFavourite(); }}>{exercise.favourite ? <Favorite color="error"/> : <FavoriteBorder/>}</Button></Stack><Stack direction="row" flexWrap="wrap" gap={0.5} sx={{mt: 0.5}}><Chip size="small" label={exercise.equipmentTags[0]}/><Chip size="small" label={exercise.movementPattern}/>{exercise.effectiveNeverSuggest && <Chip size="small" color="warning" icon={<Block/>} label="Never Suggest"/>}</Stack></CardContent></Stack></CardActionArea></Card>;
}
...
export function ExerciseDetailPage() {
    ...
    return <Layout title={exercise.name} hideNav><ScreenContainer><Stack spacing={2}><Stack direction="row" flexWrap="wrap" gap={1}><Chip label={exercise.contentStatus}/><Chip label={exercise.generatorEligible && !exercise.effectiveNeverSuggest ? 'Program eligible' : 'Not suggested automatically'} color={exercise.generatorEligible && !exercise.effectiveNeverSuggest ? 'success' : 'warning'}/><Chip label={exercise.equipmentTags.join(', ')}/></Stack>
        ...
        <Stack direction={{xs: 'column', sm: 'row'}} gap={1}><Button startIcon={exercise.favourite ? <Favorite/> : <FavoriteBorder/>} onClick={() => void toggle({favourite: !exercise.favourite})}>{exercise.favourite ? 'Favourite' : 'Add to favourites'}</Button><Button color="warning" startIcon={<Block/>} onClick={() => void toggle({neverSuggest: !exercise.effectiveNeverSuggest})}>{exercise.effectiveNeverSuggest ? 'Allow suggestions' : 'Never Suggest'}</Button></Stack>
