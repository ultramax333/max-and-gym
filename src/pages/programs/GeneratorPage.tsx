...
import {AutoAwesome, Block, ExpandMore, Favorite, FavoriteBorder, FitnessCenter, PlayArrow, Save, Schedule, Search, Tune} from '@mui/icons-material';
...
function QuickSessionBuilder() {
    ...
    return <Stack spacing={2}>
        <Paper sx={{overflow: 'hidden', borderRadius: '24px', bgcolor: '#101720'}}>
            <Box sx={{p: {xs: 2, sm: 3}, background: 'radial-gradient(circle at 100% 0%, rgba(83,199,183,.16), transparent 42%)'}}>
                <Stack direction="row" justifyContent="space-between" alignItems="flex-start" gap={2}>
                    <Box><Typography variant="overline" color="primary.main" fontWeight={850}>QUICK SESSION</Typography><Typography variant="h5" component="h2">Build a single session</Typography><Typography color="text.secondary" sx={{mt: 0.5, maxWidth: 620}}>Choose the target, available time and recovery. Generation stays local and rest is included in the estimate.</Typography></Box>
                    <Box sx={{width: 52, height: 52, borderRadius: '18px', display: {xs: 'none', sm: 'grid'}, placeItems: 'center', bgcolor: 'rgba(83,199,183,.12)', color: 'primary.main'}}><FitnessCenter/></Box>
                </Stack>
                <Stack direction={{xs: 'column', sm: 'row'}} gap={1.25} sx={{mt: 2.5}} flexWrap="wrap">
                    <FormControl fullWidth><InputLabel id="quick-zone">Body area</InputLabel><Select labelId="quick-zone" label="Body area" value={zone} onChange={(event) => { setZone(event.target.value as QuickSessionZone); setPreview(undefined); }}>{QUICK_SESSION_ZONES.map((entry) => <MenuItem key={entry.value} value={entry.value}>{entry.label}</MenuItem>)}</Select></FormControl>
                    ...
                    <FormControl fullWidth><InputLabel id="quick-goal">Training goal</InputLabel><Select labelId="quick-goal" label="Training goal" value={goal} onChange={(event) => { const next = event.target.value as SelectableGoal; setGoal(next); setSessionRestSeconds(goalRecovery[next]); setPreview(undefined); }}><MenuItem value="strength">Strength</MenuItem><MenuItem value="hypertrophy">Hypertrophy</MenuItem><MenuItem value="endurance">Endurance</MenuItem></Select></FormControl>
                ...
                <Stack direction="row" gap={0.75} flexWrap="wrap" sx={{mt: 1.5}}><Chip icon={<FitnessCenter/>} label={QUICK_SESSION_ZONES.find((entry) => entry.value === zone)?.label}/><Chip label={goal[0].toUpperCase() + goal.slice(1)}/><Chip icon={<Schedule/>} label={`${duration} min`}/><Chip variant="outlined" label={`${sessionRestSeconds < 60 ? `${sessionRestSeconds} sec` : `${sessionRestSeconds / 60} min`} rest`}/></Stack>
            </Box>
            <Divider/>
        ...
        {preview && <Stack spacing={1.5}>
            <Paper sx={{p: {xs: 2, sm: 2.5}, borderRadius: '20px', bgcolor: 'rgba(83,199,183,.06)', borderColor: 'rgba(83,199,183,.28)'}}>
...
export function GeneratorPage() {
    ...
    return <Layout title="Generator" hideNav><ScreenContainer><SectionHeader eyebrow="LOCAL · PRIVATE · OFFLINE" title="Workout generator"/><Typography color="text.secondary" sx={{mt: -1, mb: 2}}>Build a coherent session for today or a reusable weekly plan.</Typography><Paper sx={{p: 0.5, mb: 2, borderRadius: '16px'}}><ToggleButtonGroup exclusive value={mode} onChange={(_, next: 'session' | 'program' | null) => { if (next) setMode(next); }} fullWidth sx={{'& .MuiToggleButton-root': {border: 0, borderRadius: '12px !important', minHeight: 48}}}><ToggleButton value="session">Single session</ToggleButton><ToggleButton value="program">Weekly program</ToggleButton></ToggleButtonGroup></Paper>{mode === 'session' ? <QuickSessionBuilder/> : <WeeklyProgramBuilder/>}<Button sx={{mt: 2}} onClick={() => navigate('/programs')}>Back to programs</Button></ScreenContainer></Layout>;
}
