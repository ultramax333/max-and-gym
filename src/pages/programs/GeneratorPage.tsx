    ...
    return <Stack spacing={2}>
        <Paper sx={{overflow: 'hidden', borderRadius: '24px', bgcolor: '#101720'}}>
            <Box sx={{p: {xs: 2, sm: 3}, background: 'radial-gradient(circle at 100% 0%, rgba(83,199,183,.16), transparent 42%)'}}>
                ...
                <Stack direction="row" gap={0.75} flexWrap="wrap" sx={{mt: 1.5}}><Chip icon={<FitnessCenter/>} label={QUICK_SESSION_ZONES.find((entry) => entry.value === zone)?.label}/><Chip icon={<Schedule/>} label={`${duration} min`}/><Chip variant="outlined" label={`${sessionRestSeconds < 60 ? `${sessionRestSeconds} sec` : `${sessionRestSeconds / 60} min`} rest`}/></Stack>
            </Box>
            <Divider/>
