...
describe('ProgramDetailPage', () => {
    ...
    it('renders an empty two-day program with one page heading and clear actions', async () => {
        const program = await new ProgramRepository(db).create({name: 'Force locale', weeklyFrequency: 2, defaultDurationMinutes: 40});
        ...
        expect(await screen.findByRole('heading', {level: 1, name: 'Force locale'})).toBeInTheDocument();
        expect(screen.getAllByRole('button', {name: /Ajouter un exercice/})).toHaveLength(2);
    ...
    it('navigates from the creation dialog to the new builder without a route error', async () => {
        ...
        await user.click(await screen.findByRole('button', {name: 'Créer', exact: true}));
        const name = screen.getByRole('textbox', {name: 'Nom'});
        await user.clear(name);
        await user.type(name, 'Programme test');
        await user.click(screen.getByRole('dialog').querySelector('button:last-of-type')!);
        expect(await screen.findByRole('heading', {level: 1, name: 'Programme test'})).toBeInTheDocument();
    });
});
